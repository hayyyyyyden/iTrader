...
class IntradayOLSMRStrategy(Strategy):
    ...
    regression to determine the hedge ratio between a pair of equities. The
    z-score of the residuals time series is then calculated in a rolling fashion
    and if it exceeds an interval of thresholds(defaulting to [0.5, 3.0]) then
    a long/short signal pair are generated (for the high threshold) or an
    exit signal pair are generated (for the low threshold).
    """
    def __init__(
...
if __name__ == "__main__":
    csv_dir = './data/M1' # CHANGE THIS!
    symbol_list = ['AAPL', 'GOOG']
    initial_capital = 100000.0
    heartbeat = 0.0
    start_date = dt(2014, 3, 14)
    backtest = Backtest(
